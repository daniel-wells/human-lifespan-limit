...
They also used all individuals in the dataset to calculate mean age of death and used this to argue that the annual average age of supercentenarians had not increased since 1968 (the start of the dataset). I recreate their plot below but add error bars representing the standard error of the mean for each point to visualise the uncertainty in the values.
```{r warning=FALSE}
# Figure 2C
...
```
You can see that for the earlier points there are no error bars, this is because there is only a single data point for those years! It is therefore quite misleading to give each mean equal weighting by fitting a cubic spline to the means alone. Below I recreate the graph but using the whole dataset.
...
In this form the uncertainty in the first few data points is much clearer, and the dip pattern they fitted is much less convincing. I would argue that a linear regression fits the data just as well and gives an increase of 0.87 per year.

```{r}
q <- p + 
    geom_smooth(method = "lm", colour="red") + 
    annotate("text", x = 1975, y = 121,
             label = paste("gradient =",format(coef(lm(death_year~age_inyears, age.data))[2], digits=3)))
q
